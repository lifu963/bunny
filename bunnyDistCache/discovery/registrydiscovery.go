...
func (d *RegistryDiscovery) Refresh() error {
	...
	servers := strings.Split(resp.Header.Get("Servers"), ",")
	...
	d.consHash.Register(d.servers...)
	d.lastUpdate = time.Now()
	return nil
}

func (d *RegistryDiscovery) Get(group, key string) (string, error) {
	if err := d.Refresh(); err != nil {
		return "", err
	}
	return d.MultiServersDiscovery.Get(group, key)
}
