...
func genAPI(aid, cid, quality int, bvid string, bangumi bool, cookie string, requester *request.Requester) (string, error) {
	var (
		err        error
		baseAPIURL string
		params     string
...
func extractBangumi(url, html string, extractOption extractors.Options, requester *request.Requester) ([]*extractors.Data, error) {
	...
	if !extractOption.Playlist {
		...
		return []*extractors.Data{bilibiliDownload(options, extractOption, requester)}, nil
	}
	...
	for index, u := range data.EpList {
		...
		go func(index int, options bilibiliOptions, extractedData []*extractors.Data) {
			defer wgp.Done()
			extractedData[index] = bilibiliDownload(options, extractOption, requester)
...
func extractNormalVideo(url, html string, extractOption extractors.Options, requester *request.Requester) ([]*extractors.Data, error) {
	...
	if len(pageData.Sections) == 0 {
		...
		return multiPageDownload(url, html, extractOption, pageData, requester)
	}
	// handle another kind of playlist
	// https://www.bilibili.com/video/av*** each video in playlist has different av/bv id
	return multiEpisodeDownload(url, html, extractOption, pageData, requester)
}

// handle multi episode download
func multiEpisodeDownload(url, html string, extractOption extractors.Options, pageData *multiPage, requester *request.Requester) ([]*extractors.Data, error) {
...
func multiPageDownload(url, html string, extractOption extractors.Options, pageData *multiPage, requester *request.Requester) ([]*extractors.Data, error) {
...
func (e *extractor) Extract(url string, option extractors.Options, requester *request.Requester) ([]*extractors.Data, error) {
	var err error
	html, err := requester.Get(url, referer, nil)
	if err != nil {
		return nil, errors.WithStack(err)
	}
	...
	if strings.Contains(url, "bangumi") {
		// handle bangumi
		return extractBangumi(url, html, option, requester)
	}
	// handle normal video
	return extractNormalVideo(url, html, option, requester)
}

// bilibiliDownload is the download function for a single URL
func bilibiliDownload(options bilibiliOptions, extractOption extractors.Options, requester *request.Requester) *extractors.Data {
	var (
		err  error
		html string
	)
	if options.html != "" {
		...
		html, err = requester.Get(options.url, referer, nil)
		if err != nil {
	...
	api, err := genAPI(options.aid, options.cid, 127, options.bvid, options.bangumi, extractOption.Cookie, requester)
	if err != nil {
		return extractors.EmptyData(options.url, err)
	}
	jsonString, err := requester.Get(api, referer, nil)
	if err != nil {
		return extractors.EmptyData(options.url, err)
	}
	...
	if dashData.Streams.Audio != nil {
		...
		s, err := requester.Size(audios[audioID], referer)
		if err != nil {
			return extractors.EmptyData(options.url, err)
		}
	...
	for _, stream := range dashData.Streams.Video {
		s, err := requester.Size(stream.BaseURL, referer)
		if err != nil {
			return extractors.EmptyData(options.url, err)
		}
	...
	return &extractors.Data{
		...
		Captions: map[string]*extractors.CaptionPart{
			...
			"subtitle": getSubTitleCaptionPart(options.aid, options.cid, requester),
		},
		URL: options.url,
	}
}
...
func getSubTitleCaptionPart(aid int, cid int, requester *request.Requester) *extractors.CaptionPart {
	jsonString, err := requester.Get(
