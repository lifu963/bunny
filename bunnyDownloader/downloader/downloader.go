...
func New(option Options, requester *request.Requester) *Downloader {
...
func (downloader *Downloader) caption(url, fileName, ext string, transform func([]byte) ([]byte, error)) error {
	...
	body, err := downloader.requester.GetByte(url, refer, nil)
	if err != nil {
		return err
	}
...
func (downloader *Downloader) writeFile(url string, file *os.File, headers map[string]string) (int64, error) {
	res, err := downloader.requester.Request(http.MethodGet, url, nil, headers)
	if err != nil {
		return 0, err
	}
